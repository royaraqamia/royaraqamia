...
function DropdownMenuShortcut({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span
      ...
      className={cn(
        'ms-auto text-[10px] font-mono font-medium tracking-widest text-muted-foreground/70 bg-muted/60 px-1.5 py-0.5 rounded border border-border/40 shadow-2xs group-focus:text-accent-foreground group-focus:border-accent-foreground/20 transition-colors',
        className
      )}
      {...props}
    />
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuPrimitive.SubTrigger
      ...
      data-inset={inset}
      className={cn(
        'group flex cursor-pointer select-none items-center rounded-lg px-2.5 py-2 text-xs md:text-sm font-medium outline-none transition-[background-color,color,transform] duration-150 ease-out',
    ...
    >
      {children}
      <ChevronRightIcon className="ms-auto size-4 shrink-0 text-muted-foreground/80 transition-transform duration-150 rtl:rotate-180" />
    </DropdownMenuPrimitive.SubTrigger>
  );
}
  ...
  return (
    <DropdownMenuPrimitive.SubContent
      ...
      className={cn(
        ...
        'border border-border/60 shadow-2xl shadow-black/40 ring-1 ring-white/10',
        'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        'z-11000 min-w-48 max-w-[calc(100vw-2rem)] origin-(--radix-dropdown-menu-content-transform-origin) overflow-hidden rounded-2xl p-1.5',
        'contain-layout contain-style',
        className
      )}
      {...props}
