...
const alertVariants = cva(
  'group/alert relative w-full min-w-0 rounded-xl border p-4 text-sm transition-safe duration-200 ease-out grid grid-cols-[0_1fr] has-[>svg]:grid-cols-[auto_1fr] gap-y-1 has-[>svg]:gap-x-3.5 items-start [&>svg]:shrink-0 [&>svg]:size-4.5 [&>svg]:translate-y-0.5 [&>svg]:text-current [&>svg]:transition-transform [&>svg]:duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background',
  {
    variants: {
      variant: {
        default:
          'bg-card/95 text-card-foreground border-border/70 shadow-xs shadow-black/[0.03] dark:shadow-none hover:border-border/90',
        destructive:
