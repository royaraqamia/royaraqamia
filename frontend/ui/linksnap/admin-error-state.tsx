...
export function AdminErrorState({ error, onRetry }: AdminErrorStateProps) {
  return (
    <div className="bg-destructive/5 border-destructive/20 mx-auto max-w-lg space-y-4 rounded-3xl border p-6 text-center sm:p-8">
      ...
      <button
        onClick={onRetry}
        className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-xs rounded-full transition-safe inline-flex items-center gap-1.5 cursor-pointer focus-ring touch-target btn-press"
      >
