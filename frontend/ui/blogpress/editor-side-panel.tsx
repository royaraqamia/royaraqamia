    ...
    <div className="flex items-center justify-between text-xs pt-1">
      ...
      <span
        className={`font-mono text-xs tabular-nums px-2 py-0.5 rounded-md border font-bold transition-colors ${
          status === 'over'
            ? 'bg-rose-500/15 text-rose-600 border-rose-500/30 ring-1 ring-rose-500/20'
...
function SeoCheck({
  ...
  return (
    <div
      className={`group flex items-start gap-3 p-3 rounded-2xl border transition-[border-color,box-shadow] duration-200 ${
        passed
...
function StatsRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl text-xs hover:bg-muted/50 transition-colors duration-150">
      ...
      <span className="font-mono text-xs font-bold text-foreground tabular-nums bg-background/90 px-2.5 py-0.5 rounded-lg border border-border/50 shadow-xs">
        {value}
      </span>
    </div>
  );
        ...
        <div className="sticky top-0 z-20 flex flex-col border-b border-border/40 bg-background/90 px-6 sm:px-8 pt-6 pb-4">
          ...
          <div className="flex items-center p-1 rounded-2xl bg-muted/60 border border-border/40 gap-1 shadow-inner">
            <Button
              ...
              variant="ghost"
              size="sm"
              className={`flex-1 h-8 rounded-xl text-xs font-bold transition-[background-color,color,transform] duration-200 active:scale-[0.98] ${
                tab === 'seo'
                  ? 'bg-background text-foreground shadow-xs ring-1 ring-black/5 font-bold'
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    maxLength={160}
                    rows={3}
                    className="text-xs resize-none min-h-22 rounded-xl bg-card border-border/70 focus-visible:ring-2 focus-visible:ring-primary/20 focus-visible:border-primary transition-[border-color,box-shadow] duration-200 shadow-xs leading-relaxed placeholder:text-muted-foreground/50"
                  />
                  <CounterBadge
                    label="عدد الحروف"
                ...
                {(metaTitle || metaDesc) && (
                  <div className="space-y-2 pt-2">
                    ...
                    <div className="rounded-2xl border border-border/60 bg-linear-to-b from-card via-card to-muted/20 p-4 space-y-2 shadow-xs hover:shadow-md transition-[box-shadow] duration-300">
                      <div
              ...
              <section className="rounded-2xl border border-border/60 bg-card/60 p-4 space-y-3 shadow-xs hover:border-border/80 transition-[border-color,box-shadow] duration-200">
                ...
                <div className="space-y-1.5">
                  <div className="h-2.5 w-full rounded-full bg-muted/80 overflow-hidden p-0.5 ring-1 ring-border/20">
                    <div
                      className={`h-full rounded-full transition-[width] duration-500 ease-out ${readabilityColor}`}
                  ...
                  <div className="flex items-center justify-between text-[11px] text-muted-foreground/80 font-medium pt-0.5">
                    <span>
                      النَّتيجة:{' '}
                      <strong className="text-foreground font-mono">{readability.score}</strong>
                      /100
                    </span>
