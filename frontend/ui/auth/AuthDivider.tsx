...
export function AuthDivider({ children = 'أو', className = '' }: AuthDividerProps = {}) {
  return (
    ...
    >
      ...
      <span
        dir="auto"
        className="inline-flex items-center justify-center rounded-full border border-border/80 bg-card/75 px-3.5 py-1 text-[11px] font-bold tracking-wider text-muted-foreground/90 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-safe duration-300 ease-out hover:scale-[1.02] hover:border-border hover:bg-card/90 hover:text-foreground active:scale-[0.98]"
      >
        {children}
      </span>
