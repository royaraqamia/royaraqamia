...
| P2       | **Export post (Markdown/HTML) + duplicate post**                                      | Content portability                                                                                                          | ⬜ TODO — posts-service is CRUD-only; no export/duplicate fn                                                                                                      |
| P2       | **Reading-progress / focus mode**                                                     | Calm writing environment                                                                                                     | 🔶 PARTIAL — focus mode done in `editor-content.tsx` (وضع التركيز); reading-progress only exists on public article page, not in editor                            |
...
| Priority | Feature                                                               | UX rationale                                     | Files / layers                                                                                                                                 |
| -------- | --------------------------------------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| P1       | **Share sheet + copy feedback** (toast) per row                       | Immediate gratification, mobile-friendly sharing | 🔶 PARTIAL — per-row copy + Sonner toast done (`link-row-card.tsx` handleCopy); share sheet only exists on shorten-success screen, not per row |
| P2       | **Password-protected links** (per-link secret, RLS-safe)              | Private/campaign links                           | ⬜ TODO — no `password_hash`; redirect flow has no unlock step; add migration + redirect check + edit form                                     |
| P2       | **Custom slugs validation UX** — live availability check while typing | Error prevention before submit                   | ⬜ TODO — server-side-only collision check (`shorten-url.ts`); no debounced availability endpoint; `link-edit-form.tsx` has no slug field      |
...
| P1       | **Weekly/monthly targets per habit** (goal), not just daily/weekly frequency                          | Measures progress, not just completion            | ⬜ TODO — no `habits.target` column/contract field or stats progress; add migration + repo/service + edit-modal field                                                                                                                                                                                          |
...
| P1       | **Custom reminder time per habit** (surface existing notification infra)                              | Reminders that fit daily rhythm                   | ⬜ TODO — reminders fixed at 07:00 UTC (`20260805162137`); no per-habit time/UI; add `habits.reminder_time` + cron changes                                                                                                                                                                                     |
...
| P1       | **CSV import/export**                                                              | Onboarding + backup + portability                                   | ⬜ TODO — only LinkSnap has CSV; add repo/service fn + dialog                                                                        |
