...
| P2       | **Keyboard-first**: `/` slash commands, save/check shortcuts                               | 🔶 PARTIAL — ⌘/Ctrl+S ✅ (BlogPress editor: `use-post-autosave` keydown listener forces an immediate save of pending changes, reusing the existing debounce/`lastSaved` indicator); existing `n` shortcut kept. `/` slash commands **dropped / out-of-scope** (diminishing returns vs. toolbar + Cmd+K; not worth the input-context complexity).                                                                                                                                                             |
