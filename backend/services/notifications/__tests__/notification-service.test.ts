...
describe('NotificationService', () => {
  ...
  describe('broadcast', () => {
    it('writes one batched insert for the resolved targets', async () => {
      const { repository, service } = makeDeps();
      (repository.broadcast as ReturnType<typeof vi.fn>).mockResolvedValue(3);
    ...
    it('returns 0 (no-op) when the target list is empty', async () => {
      const { repository, service } = makeDeps();
