...
vi.mock('@/backend/repositories/otp/supabase-otp-repository', () => ({
...
vi.mock('@/backend/clients/rate-limiter', () => ({
...
describe('signup', () => {
  it('returns validation error for invalid data', async () => {
    const { signup } = await import('@/backend/controllers/auth');
  ...
  it('creates user record, generates OTP, and redirects on success', async () => {
    ...
    const { createOtpRecord } = await import('@/backend/repositories/otp/supabase-otp-repository');
...
describe('login', () => {
  it('returns validation error for invalid data', async () => {
    const { login } = await import('@/backend/controllers/auth');
...
describe('verifyOtp', () => {
  it('returns error when OTP record not found', async () => {
    const { verifyOtpRecord } = await import('@/backend/repositories/otp/supabase-otp-repository');
    vi.mocked(verifyOtpRecord).mockResolvedValue({ error: 'لم يتم العثور على رمز التحقق' });

    const { verifyOtp } = await import('@/backend/controllers/auth');
...
describe('resendOtp', () => {
  it('returns rate limit error', async () => {
    ...
    const { resendOtp } = await import('@/backend/controllers/auth');
...
describe('resetPassword', () => {
  it('returns rate limit error', async () => {
    ...
    const { resetPassword } = await import('@/backend/controllers/auth');
...
describe('updatePassword', () => {
  it('returns error if passwords do not match', async () => {
    const { updatePassword } = await import('@/backend/controllers/auth');
...
describe('logout', () => {
  it('signs out and redirects to home', async () => {
    ...
    const { logout } = await import('@/backend/controllers/auth');
...
describe('signInWithGoogle', () => {
  it('redirects to OAuth URL on success', async () => {
    ...
    const { signInWithGoogle } = await import('@/backend/controllers/auth');
...
describe('signInWithOAuth', () => {
  it('redirects to OAuth URL with google provider and next param', async () => {
    ...
    const { signInWithOAuth } = await import('@/backend/controllers/auth');
