...
function observationText(observation) {
  return [observation?.summary, observation?.evidence, observation?.suggestedAction]
    .filter(Boolean)
    .join(' ');
}
...
function hasUnsupportedRuntimeRootCauseClaim(observation) {
  const text = observationText(observation);
  if (!/\b(?:caused by|root cause|responsible for failures|would produce)\b/i.test(text))
    return false;
