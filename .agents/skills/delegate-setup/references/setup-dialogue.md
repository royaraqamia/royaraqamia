...
- **Questions 2 and 3 are set-valued.** The answer is a mapping across CLIs, so options must span the
  _discovered_ CLIs — never an arbitrary subset — with both directions expressible: burn _and_ spare,
  trust _and_ burned-by. Multi-select if the harness has it; otherwise ask in prose.
