...
export function loadEffective(cwd = process.cwd()) {
  ...
  return {
    ...
    lanes: Object.fromEntries(
      Object.entries(effective).map(([name, { lane, source }]) => [name, { ...lane, source }])
    ),
  };
}
...
export function writeAtomic(targetPath, document) {
  ...
  const tmp = join(dirname(targetPath), `.config.${process.pid}.${Date.now()}.tmp`);
...
function main(argv) {
  try {
    ...
    if (cmd === 'validate') {
      const file = argv.find((a, i) => i > 0 && !a.startsWith('--') && argv[i - 1] !== '--cwd');
      if (!file) fail('validate needs a file path');
      const parsed = parseConfigDocument(readFileSync(resolve(file), 'utf8'), file);
      if (!parsed.ok) fail(parsed.error);
      process.stdout.write(
        `${JSON.stringify({ ok: true, path: resolve(file), lanes: Object.keys(parsed.document.lanes) }, null, 2)}\n`
      );
      return;
    }
    ...
    if (cmd === 'write') {
      ...
      const parsed = parseConfigDocument(readFileSync(resolve(file), 'utf8'), file);
      if (!parsed.ok) fail(parsed.error);
      const target = scope === 'global' ? globalConfigPath() : assertSafeProjectConfigPath(cwd);
