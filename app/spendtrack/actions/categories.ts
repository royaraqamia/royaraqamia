...
export async function createCategory(_prevState: ActionState, formData: FormData) {
  ...
  try {
    await createSpendtrackRepository(supabase).createCategory({
      user_id: user.id,
      name,
      color_hex,
    });
...
export async function updateCategory(
  ...
  try {
    await createSpendtrackRepository(supabase).updateCategory(categoryId, user.id, {
      name,
      color_hex,
    });
  } catch (error) {
...
export async function deleteCategory(categoryId: string, _prevState: ActionState) {
  ...
  try {
    await createSpendtrackRepository(supabase).deleteCategory(categoryId, user.id);
  } catch (error) {
