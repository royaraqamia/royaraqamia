  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm transition-all duration-200 hover:border-indigo-100 dark:hover:border-indigo-800">
      <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        {editingCode === code ? (
          ...
          <div className="space-y-1.5 min-w-0 flex-1">
            <div className="flex items-center gap-2.5">
              <button
                type="button"
                className="text-sm font-mono font-bold text-indigo-600 hover:underline cursor-pointer shrink-0 bg-transparent border-none p-0 focus-ring"
                onClick={() => window.open(fullShortUrl, '_blank')}
              >
                /{code}
              </button>
              <span className="text-xs text-slate-500 dark:text-slate-500 bg-slate-50 dark:bg-slate-900/50 px-2 py-0.5 rounded-full font-medium font-mono flex items-center gap-1 shrink-0">
              ...
              {isBlocked && (
                <span className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-100 dark:border-red-800 px-2 py-0.5 rounded-full font-bold">
        ...
        <div className="flex items-center gap-2 shrink-0 self-end md:self-auto">
          <button
            ...
            className="p-3.5 bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-600 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-lg transition-colors cursor-pointer press-scale focus-ring"
            title="نسخ الرابط"
          >
            {copiedCode === code ? (
          ...
          <button
            ...
            className={cn(
              'px-3 py-2.5 border rounded-lg font-medium text-xs flex items-center gap-1.5 transition-colors cursor-pointer press-scale focus-ring',
              isExpanded
      ...
      {deleteError && (
        <div
          aria-live="polite"
          className="mx-5 mb-4 p-2.5 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 text-red-600 dark:text-red-400 text-xs rounded-lg flex items-center gap-1.5"
        >
