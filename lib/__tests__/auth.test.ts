...
describe('signup', () => {
  ...
  it('creates user record, generates OTP, and redirects on success', async () => {
    ...
    const { sendOtpEmail } = await import('@/infrastructure/email/resend');
    ...
    const { createOtpRecord } = await import('@/lib/otp/repository');
...
describe('verifyOtp', () => {
  it('returns error when OTP record not found', async () => {
    const { verifyOtpRecord } = await import('@/lib/otp/repository');
